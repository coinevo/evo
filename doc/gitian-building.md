Gitian building
================
*Setup instructions for a Gitian build of Evo Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Evo
...
- [Building Evo Core](#building-evo-core)
...
Clone the git repositories for evo and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/coinevo/evo --recursive
git clone https://github.com/coinevo/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/evo/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
There will be a lot of warnings printed during the build of the 2 images. These can be ignored. The Xenial image is only used for Windows builds.
...
in the evo repository under 'Fetch and create inputs' to install sources which require
...
To build Evo Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the evo repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/evo/.git/
...
For example:
```bash
URL=https://github.com/laanwj/evo.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit evo=${COMMIT} --url evo=${URL} ../evo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit evo=${COMMIT} --url evo=${URL} ../evo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit evo=${COMMIT} --url evo=${URL} ../evo/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-evo:
./bin/gbuild --commit evo=${COMMIT},cpp-eth-evo=${ETHCOMMIT} --url evo=${URL},cpp-eth-evo=${ETHURL} ../evo/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the evo git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../evo/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/coinevo/evo-detached-sigs.git

BTCPATH=/some/root/path/evo
SIGPATH=/some/root/path/evo-detached-sigs

./bin/gbuild --url evo=${BTCPATH},signature=${SIGPATH} ../evo/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/evo-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/evo-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/evo-osx-build.assert
```
...
[coinevo/gitian.sigs](https://github.com/coinevo/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@coinevo.tech) and he will commit them.
