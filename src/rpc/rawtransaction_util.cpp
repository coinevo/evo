        ...
        if (name_ == "data") {
            ...
            if (Contract.exists("senderAddress")){
                ...
                if (!IsValidDestination(senderAddress))
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Evo address to send from");
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Evo address: ") + name_);
            }
