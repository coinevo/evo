...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // evo
    uint256 hashUTXORoot; // evo
    ...
    void SetNull()
    {
        ...
        nBits          = 0;
        nNonce         = 0;
        hashStateRoot  = uint256(); // evo
        hashUTXORoot   = uint256(); // evo
    ...
    explicit CBlockIndex(const CBlockHeader& block)
    {
        ...
        nNonce         = block.nNonce;
        nMoneySupply   = 0;
        hashStateRoot  = block.hashStateRoot; // evo
        hashUTXORoot   = block.hashUTXORoot; // evo
        nStakeModifier = uint256();
        hashProof = uint256(); 
        prevoutStake   = block.prevoutStake; // evo
        vchBlockSigDlgt    = block.vchBlockSigDlgt; // evo
    }
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // evo
        block.hashUTXORoot   = hashUTXORoot; // evo
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // evo
        READWRITE(hashUTXORoot); // evo
        READWRITE(nStakeModifier);
        READWRITE(prevoutStake);
        READWRITE(hashProof);
        READWRITE(vchBlockSigDlgt); // evo
    }
