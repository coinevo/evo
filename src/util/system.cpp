...
const char * const BITCOIN_CONF_FILENAME = "evo.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Evo
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Evo
    // Mac: ~/Library/Application Support/Evo
    // Unix: ~/.evo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Evo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Evo";
#else
    // Unix
    return pathRet / ".evo";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Coinevo Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Coinevo Core Developers";
    }
