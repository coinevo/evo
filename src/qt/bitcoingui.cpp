...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a evo address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and evo: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your evo addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified evo addresses"));
    ...
    openAction->setStatusTip(tr("Open a evo: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible evo command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    if(_clientModel)
    {
        ...
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && qtumVersionChecker->newVersionAvailable())
        {
            ...
            QString message(tr("New version of evo wallet is available on the evo source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to evo network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
